...
description: Your Quiz の repository tooling を操作し、setup、git workflow、mise、npm/pnpm scripts、TypeSpec commands、Markdown/TypeScript/TypeSpec conventions、generated artifacts、validation commands、安全な command boundary を扱うときに使う。
license: MIT
metadata:
  author: personal
  version: "0.1.0"
compatibility: Your Quiz tooling docs と対象 repository にアクセスできること。
---
...
- git state と uncommitted changes
- command が file を書くか、network にアクセスするか、approval が必要か
...
- 選択した command と理由
- 変更が想定される file/artifact
- validation result
- risk、approval、または skipped check

## ガードレール

- Secrets、tokens、machine-specific paths を追加しない。
- generated file は文書化された command で更新する。
